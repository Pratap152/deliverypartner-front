 ...
 *
 * This allows spaces while typing so Android TextInput
 * is not forced to change its value during composition.
 */
...
const BankAC = ({ navigation }) => {
  ...
  const fetchBankDetails = async () => {
    try {
      ...
      if (res?.data?.success) {
        ...
        /*
         * Clean backend values before displaying.
         *
         * This cleanup happens only when receiving
         * backend data. It does NOT modify the value
         * on every keyboard keystroke.
         */
  ...
  const holderValid = validateName(
    bankDetails.accountHolderName,
  );

  const bankNameValid = validateName(
    bankDetails.bankName,
  );

  const branchValid = validateName(
    bankDetails.branch,
  );
  ...
  const liveErrors = {
    ...
    bankName:
      ...
      !bankNameValid
        ? getNameError(
            bankDetails.bankName,
            FIELD_LABELS.bankName,
          )
        : '',

    branch:
      bankDetails.branch.length > 0 &&
      !branchValid
        ? getNameError(
            bankDetails.branch,
            FIELD_LABELS.branch,
...
========================================================= */
  ...
  const handleInputChange = (key, text) => {
    ...
    if (key === 'accountNumber') {
      /*
       * Do not rewrite the input unnecessarily.
       *
       * Only accept digits.
       */
    ...
    if (key === 'ifscCode') {
      /*
       * IFSC is automatically converted to uppercase.
       */
    ...
    if (
       ...
       * here.
       *
       * Android TextInput can send intermediate
       * composing states while the user types.
       *
       * We accept the text if it contains only
       ...
       * are allowed.
       *
       * Validation handles the final rules.
       */

      if (!NAME_INPUT_REGEX.test(text)) {
        return;
      }
  ...
  const handleUpdateBankDetails = async () => {
    /*
     * Trim only when saving.
     *
     * This means temporary typing states such as:
     *
     * "John "
     *
  ...
  const handleEditToggle = async () => {
    if (isEditing) {
      /*
       * Cancel editing:
       * close dropdown and reload original
       * backend values.
       */
              ...
              <View style={styles.infoContainer}>
                ...
                {showInfo && (
                  <View style={styles.tooltip}>
                    ...
                    >
                      Your bank details are securely
                      stored and used only for payouts.
                    </Text>
                  </View>
                )}
              </View>
